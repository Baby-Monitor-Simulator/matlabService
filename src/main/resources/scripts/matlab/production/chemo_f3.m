...
if ico==2
    ...
    D_V=            5;            %[s]
    D_Ta0=          2;          %[s]
    G_Ts=           -0.13;%-0.070;% -0.072;%       %[s^2/spikes] van -0.13
    G_Tv=           0.09;%0.050;%         %[s^2/spikes]
    G_Rp=           15;         %[kPa.ms/ml/spikes] .144 bij 47%
    G_Rc=           3;
    G_Rum=          3; 
    G_Rb=           1.4e5;
    G_Vp=           -0.57*fVv0/3.15;%0.115*0.72*fV0(nfv);
    ...
    f_sp(1:3)=      21;   %SS at PfO20
    f_sh(1:3)=      12.3;    %SS at PfO20
    f_v(1:3)=       6;     %SS at PfO20
    f_ac(1:3)=      9.4;%fi_ac0; 
...
elseif ico>3
    ...
    jsearch2=j2;%round(jsearch1-ftcycle(fcycle-2)/dt);
    ...
    sigma_br(ico)=(500+3*Rbrmc*exp((Cfa(ico-1)-0.12)./0.018))./(1+exp((Cfa(ico-1)-0.12)./0.018));
    fRbr2(ico)=min(Rbrmc,(fRbr2(ico-1)*tau_ox+sigma_br(ico)*tso)/(tau_ox+tso));
    fR(sbrmc)=fRbr2(ico);
    
    %Calculate fire rate baro from mean arterial pressure
    pwig(ico)=mfpart(ico);%(mfpart(ico)*(tso/tau_pb)+(tau_zb/tau_pb)*(mfpart(ico)-mfpart(ico-1))+pwig(ico-1))/(1+tso/tau_pb);
    f_ab(ico)=(f_abmin+f_abmax*exp((pwig(ico)-pn)/k_ab))/(1+exp((pwig(ico)-pn)/k_ab));
    
    
    f_v(ico)=(f_ev0+f_evinf*exp((f_ab(ico)-f_ab0)/k_ev))/(1+exp((f_ab(ico)-f_ab0)/k_ev))+W_cv*f_ac(ico)-W_pv*f_ap(ico)-theta_v;
    %vag_comp1(ico)=max(0,(Pfbrmc0-Pfbrmc(ico-1))/(0.2*Pfbrmc0)); % Ergo max of 20% is responsible for 30 bpm decrease in FHR
    vag_comp0(ico)=(-0.1+2*exp((Pfbrmc00-Pfbrmc(ico-1))./k))./(1+exp((Pfbrmc00-Pfbrmc(ico-1))./k));
...
%     f_sh1(ico)=(0+40*exp((NEa1-30e-3)./1.5e-2))./(1+exp((NEa1-30e-3)./1.5e-2));
    ...
    Ta0test(ico)=DeltaTa0(ico)+0.67*fTa0;
end
