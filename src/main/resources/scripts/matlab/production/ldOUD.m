...
if con==0
    ...
    randinter(con)=1000*round(600/dt);           % Random gaussian contraction interval in ic's
    ...
    j_contrstart(con)=round(55*mtcycle(1));               % Start time of contraction after about 30 maternal beats
...
%mR(sutv)=(Rutv+c1*Rutv0*exp((fpcon(j)-fpmax)/c2))/(1+exp((fpcon(j)-fpmax)/c2));
mR6(j)=mR(sutmc);
...
if reg==0 && j==j_contrstop(con)
        ...
        j_contrstop(con)=j_contrstart(con)+contrlength(con);  % Stop time of contraction                                       
elseif reg==1 && j==j_contrstop(con)
        con=con+1;                                              % Contraction number
