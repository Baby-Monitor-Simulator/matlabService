...
% of current past period)
if umbilical==1
    dfVummcdto=0;%(max(fVsav(num,jsearch2:jsearch1))-max(fVsav(num,jsearch1:jsearch0)))*1e-6/(ftcycle(fcycle)*dt*1e-3);
elseif umbilical==2
...
dfVmcdto=0;%(max(fVsav(nfv,jsearch2:jsearch1))-max(fVsav(nfv,jsearch1:jsearch0)))*1e-6/(ftcycle(fcycle)*dt*1e-3);
dfVbrdto=0;%(max(fVsav(nbr,jsearch2:jsearch1))-max(fVsav(nbr,jsearch1:jsearch0)))*1e-6/(ftcycle(fcycle)*dt*1e-3);
dfVadto=0;%(max(sum(fVsav(nflv:nfa,jsearch2:jsearch1)))-max(sum(fVsav(nflv:nfa,jsearch2:jsearch1))))*1e-6/(ftcycle(fcycle)*dt*1e-3);
...
% blood volumes over last period
if umbilical==1
    mfVummc=7*1e-6;%mean(fVsav(num,jsearch1:jsearch0))*1e-6;  %[m3 blood]
elseif umbilical==2
    mfVuma=mean(fVsav(numa,jsearch1:jsearch0))*1e-6;  %[m3 blood]
    mfVummc=mean(fVsav(numv,jsearch1:jsearch0))*1e-6;  %[m3 blood]
end
mfVmc=15*1e-6;%mean(fVsav(nfv,jsearch1:jsearch0))*1e-6;    %[m3 blood]
mfVbr=1e-6;%mean(fVsav(nbr,jsearch1:jsearch0))*1e-6;     %[m3 blood]
mfVa=307e-6;%mean(sum(fVsav(nflv:nfa,jsearch1:jsearch0)))*1e-6;     %[m3 blood]
...
qtot=(mfqbr+mfqmc+(1-SFf)*mfqummc);
Cfin=(Cfbr(ico-1)*mfqbr+Cfmc(ico-1)*mfqmc+(1-SFf)*mfqummc*Cummc(ico))/qtot;
Cfa(ico)=qtot/mfVa*(Cfin-Cfa(ico-1))*tso+Cfa(ico-1);
