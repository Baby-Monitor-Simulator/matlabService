...
if ico==2
    ...
    pn=             40;          %[mmHg]
    if lamb==1; pn=43; end
    k_ab=           5*4.09;      %[11.76/92=k_ab/32 mmHg]
    k_ev=           7.06;    %[spikes/s]
    W_v=            1;
    W_bsp=          1;   
    W_csp=          5;
    ...
    f_sp(1:3)=      8.3;   %SS at PfO20
    f_sh(1:3)=      11;    %SS at PfO20
...
%     jsearch1=j8;
    ...
    f_ab(ico)=(f_ab(ico-1)*tau_zb+f_ab1(ico)*tso)/(tau_zb+tso);
    ...
    vag_comp0(ico)=(-0.1+6*exp((Pfbrmc00-Pfbrmc(ico-1))./k))./(1+exp((Pfbrmc00-Pfbrmc(ico-1))./k));
    ...
    f_sp1(ico)=(0+10*exp((CAf(ico)-10e-3)./2e-3))./(1+exp((CAf(ico)-10e-3)./2e-3));
    f_sh1(ico)=(0+40*exp((CAf(ico)-40e-3)./7e-3))./(1+exp((CAf(ico)-40e-3)./7e-3));
            
    if ico>3
        f_sp(ico)=f_sp(ico);%+f_sp1(ico);
        f_sh(ico)=f_sh(ico);%+f_sh1(ico);
    end
